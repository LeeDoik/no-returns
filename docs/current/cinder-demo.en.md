...
- [CarryThreat.cs](../../NoReturns/Assets/_NoReturns/Runtime/CarryLab/CarryThreat.cs): three A/B/C listeners and one outer creature. Wall-aware navigation covers the larger map and is prepared before connection. Subsequent shifts reuse the navigation graph for this static map. See the [dense facility specification](cinder-density.en.md) for placement and shared state.
...
- [CarryRoom.Network.cs](../../NoReturns/Assets/_NoReturns/Runtime/CarryLab/CarryRoom.Network.cs): host authority, preparation-only joining, Cinder protocol 12 versus existing trial protocol 10. Builds for different maps cannot join each other.
- The maintenance clue moved to the office. Nine buildings now contain rooms and corridors. New monster species, final art and additional cargo types remain outside this change.
