...
1. Double-click PLAY.cmd (`../../PLAY.cmd`; retired file) in the project folder.
...
2. Run PLAY_TWO.cmd (`../../PLAY_TWO.cmd`; retired file). It opens a host window and a guest window.
...
Run EDIT.cmd (`../../EDIT.cmd`; retired file) to open the project in Godot. **F6 runs the selected scene**, while **F5 runs the project**. The entry scene is `scenes/main.tscn`; depot geometry and UI are generated by GDScript, so the edit viewport does not preview every generated object.
