...
public sealed class ReceiptFeedback : MonoBehaviour {
 ...
 void Awake(){
  var offset=CinderDemoLayout.ReceiptOffset;
  signal=CarryWorld.Mat(new Color(.9f,.55f,.08f));signal.EnableKeyword("_EMISSION");
  for(int i=0;i<2;i++){
   Visual("Reception floor outline",new Vector3(-6,.014f,8+i*2)+offset,new Vector3(3,.008f,.07f),signal).transform.SetParent(transform);
   Visual("Reception floor outline",new Vector3(-7.5f+i*3,.014f,9)+offset,new Vector3(.07f,.008f,2),signal).transform.SetParent(transform);
  }
  scan=Visual("Reception scanner",new Vector3(-6,.02f,9)+offset,new Vector3(2.85f,.008f,.045f),signal).transform;scan.SetParent(transform);
  ...
  sound.transform.position=new Vector3(-8.1f,1,9)+offset;
 ...
 public static Vector3 Slot=>new Vector3(-7.96f,.84f,8.691f)+CinderDemoLayout.ReceiptOffset;
 ...
 public void Display(int phase,float progress,bool collected,bool ready){
  ...
  scan.gameObject.SetActive(state==1);scan.position=new Vector3(-6,.024f,Mathf.Lerp(8.08f,9.92f,progress))+CinderDemoLayout.ReceiptOffset;
