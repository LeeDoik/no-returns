...
public static class ShipInteriorTrialBuild {
    ...
    public static void Create() {
        ...
        foreach(float lane in new[]{-.55f,0,.55f}){
            controller.enabled=false;test.transform.position=new Vector3(lane,.05f,-8);controller.enabled=true;
            for(int i=0;i<300;i++)controller.Move(new Vector3(0,-.04f,.04f));
