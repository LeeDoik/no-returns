...
func begin() -> void:
	...
	if role == "host":
		...
		game.host_game(27951, true)
	else:
		game.session.protocol = 999
		game.join_game("127.0.0.1", 27951)
func _process(_delta: float) -> bool:
	if not game: return false
	if Time.get_ticks_msec() > deadline:
		push_error("Protocol mismatch test timed out")
		quit(1)
	elif role == "guest" and game.notice_key == "version_mismatch":
		if game.session.confirmed or not game.workers.is_empty():
			push_error("Mismatched guest admitted")
			quit(1)
		else:
			print("PASS network guest: incompatible version rejected before admission")
			quit(0)
