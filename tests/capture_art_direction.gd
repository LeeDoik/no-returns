...
func _run() -> void:
	...
	for view in [["directed-intake",Vector3(0,0.05,2),0.0], ["directed-archive",Vector3(-21,0.05,-15),0.0], ["directed-wind",Vector3(16,0.05,-24),-PI/2], ["directed-dispatch",Vector3(-10,0.05,-41),PI/2]]:
	...
	await settle(20); await capture("directed-carry")
	...
	await settle(20); await capture("directed-wind-en")
	...
	await settle(10); await capture("directed-overhead")
	game.queue_free(); await process_frame
	print("ART DIRECTION CAPTURE %s" % ("PASS" if failures == 0 else "FAIL")); quit(1 if failures else 0)
