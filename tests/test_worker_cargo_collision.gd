...
func run() -> void:
	...
	for i in range(40):
		...
		await frames(1)
	check(worker.position.z < 4.2 and cargo.rules.holder_id == 1, "own held crate does not block carrying: worker=%s cargo=%s holder=%d" % [worker.position, cargo.body.position, cargo.rules.holder_id])
	game._add_worker(22)
