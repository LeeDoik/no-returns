...
func _run() -> void:
	...
	await _held_wall()
	await _throw_case("floor", Vector3(-7, 0.05, 5), 0.0, Vector3.UP)
	await _throw_case("oblique floor", Vector3(-5, 0.05, 5), 0.15, Vector3.UP)
	await _throw_case("back wall", Vector3(0, 0.05, -8), 0.0, Vector3.BACK)
	await _throw_case("side wall", Vector3(7, 0.05, 3), -PI / 2.0, Vector3.LEFT)
