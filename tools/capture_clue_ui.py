...
OUT=ROOT/'artifacts/space-play-06-ui'
...
try:
 command('host',quiet=True);hp=start('host');wait('visible host started',lambda:state('host').get('hazard'))
 command('host',action=True);time.sleep(.4);command('host',action=True);wait('arrived',lambda:state('host').get('phase')==2)
 for x,z in [(6,-2),(6,0)]:go('host',x,z,quiet=False)
 command('host',quiet=True,yaw=90,pitch=7,inspect=True);wait('first clue recorded',lambda:state('host').get('clueMask')==1)
 command('host',quiet=True,yaw=90,pitch=7,journal=True);time.sleep(.5)
 command('host',quiet=True,yaw=90,pitch=7,capture=True);wait('screen written',lambda:(folders['host']/'screen.png').exists())
 time.sleep(.5);print('CAPTURE',folders['host']/'screen.png',flush=True)
 (OUT/'latest.json').write_text(json.dumps({'run':str(run),'capture':str(folders['host']/'screen.png'),'scope':'Visible Windows player capture; inspect pixels separately'},indent=2))
finally:
 for p in procs:
