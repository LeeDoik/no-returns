...
OUT=ROOT/'artifacts/space-play-03-solo'
...
try:
 command('host',quiet=True);hp=start('host');wait('solo hazard starts',lambda:state('host').get('hazard') is True)
 command('host',action=True);time.sleep(.4);command('host',action=True);wait('solo arrives',lambda:state('host').get('phase')==2)
 go('host',1,-2);go('host',1,3.4)
 command('host',quiet=True,call=True)
 wait('solo attack warning',lambda:danger().get('state')==2)
 wait('solo down',lambda:danger().get('down0'))
 wait('solo automatically recovered',lambda:state('host').get('phase')==4 and danger().get('evacuations',0)==1 and not danger().get('down0'))
 assert state('host')['credits']==0;checks.append('solo has no unearned payment')
 report={'status':'PASS','checks':checks,'run':str(run),'scope':'One real executable, ordinary inputs; no human balance approval'}
