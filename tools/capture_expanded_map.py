...
OUT=ROOT/'artifacts/space-play-07-ui'
...
try:
 command('host',quiet=True);start('host');wait('visible expanded map',lambda:state('host').get('hazard'),seconds=60)
 for x,z in [(10,-2),(10,8),(13,16)]:go('host',x,z,quiet=False)
 command('host',quiet=True,yaw=0,pitch=0,capture=True)
 wait('east corridor screen saved',lambda:(folders['host']/'screen.png').exists());time.sleep(.5)
 import shutil
 shutil.copy2(folders['host']/'screen.png',run/'east-corridor.png')
 for x,z in [(13,27),(13,29),(8,29),(4,29)]:go('host',x,z,quiet=False)
 stamp=(folders['host']/'screen.png').stat().st_mtime
 command('host',quiet=True,yaw=-120,pitch=3,capture=True)
 wait('north detour screen saved',lambda:(folders['host']/'screen.png').stat().st_mtime>stamp);time.sleep(.5)
 shutil.copy2(folders['host']/'screen.png',run/'north-detour.png')
 (OUT/'latest.json').write_text(json.dumps({'run':str(run),'scope':'Visible Windows captures; inspect pixels separately'},indent=2));print(run,flush=True)
finally:
 for p in procs:
