...
class HazardTest(DemoTest):
    def __init__(self, args):
        self.args = args
        self.output = ROOT / 'artifacts/cinder-demo-hazards'
    ...
    def exercise(self):
        ...
        # Approach north of STORAGE (north wall z=-25.8), west of its wall first.
        # Keep the rescue partner three metres west until the first attack.
        for i, endpoint in ((1, -9), (0, -6)):
            for x, z in [(-38.2, -30), (-32.8, -30), (-20, -30), (-20, -23.5), (endpoint, -23.5), (endpoint, -16.8)]:
                self.go(x, z, i=i)
        pulse = [0]
        def warned():
            ...
            return self.replicated_danger(lambda d: d.get('state') == 2 and not d.get('down', [False] * 4)[0])
        self.wait('listener warning precedes host down on both peers', warned, seconds=90)
        self.command(0)
